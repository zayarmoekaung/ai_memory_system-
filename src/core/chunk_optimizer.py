...
if __name__ == "__main__":
    ...
    total_optimized_tokens = query_tokens_mock + sum(chunk_optimizer.count_tokens(c['content']) for c in optimized_chunks)
    print(f"Total tokens after optimization: {total_optimized_tokens}")

    for i, chunk in enumerate(optimized_chunks):
        print(f"  Optimized Chunk {i+1} (Score: {chunk.get('score', 'N/A')}, Tokens: {chunk_optimizer.count_tokens(chunk['content'])}): {chunk['content']}")
